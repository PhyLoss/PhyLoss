				...
				for line in f:
					if line.startswith("pid"):
						...
						for cat in cats:
							...
							if not GO and (cat == "R" or cat == "S"): continue
							# if GO function, remove "GO:"
...
def getStrFunction(dictFunct, STRICT, numMembers, dictFunctAllClusters):
	strFunct = ""
	...
	if len(dictFunct) > 0: # if there are some functions assigned to a cluster
		...
		for funct in dictFunct: # for functions annotated for at least half of members
			if dictFunct[funct] >= threshold: 
				...
				strFunct += funct + "\t" + str(dictFunct[funct]) + "\t" + str(thresholdNumFunctions) + "\n"
