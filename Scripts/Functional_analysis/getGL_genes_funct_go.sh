...
OUTPUT_DIR=$WORKING_DIR/ClusterAnalysis/GO
...
do
	...
	echo python3 getGL_genes_funct.py -f $SPECIES -i $TSV_DIR/$C_DIR/db_clu_all.tsv -p $PARENTS_DIR/ -l $LCA_FILE -e $EMAPPER_DIR -G -g $GENES_FILE -s $SUMMARY_FILE -o $OUTPUT_FILE > out_funct_go_0_$c.txt
	time python3 getGL_genes_funct.py -f $SPECIES -i $TSV_DIR/$C_DIR/db_clu_all.tsv -p $PARENTS_DIR/ -l $LCA_FILE -e $EMAPPER_DIR -G -g $GENES_FILE -s $SUMMARY_FILE -o $OUTPUT_FILE > out_funct_go_0_$c.txt
done
echo "Done."
