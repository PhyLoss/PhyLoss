...
def parseArgv(argv):
	...
	options = "hf:p:i:l:o:s:g:" # options that require arguments should be followed by :
	long_options = ["help", "focal_species_id", "parents_directory", "input_tsv_file", "lca_file", "output_file", "summary_file", "genes_file"]
	try:
		opts, args = getopt.getopt(argv, options, long_options)
	except getopt.GetoptError:
		print("Error. Usage: getGL_genes.py -f <focal_species_id> -p <parents_directory> -i <input_tsv_file> -l <lca_file> -o <output_file> -s <summary_file> -g <genes_file>")
		sys.exit(2)
	...
	for opt, arg in opts:
		print(opt + "\t" + arg)
		if opt in ("-h", "--help"):
			print("Usage: getGL_genes.py -f <focal_species_id> -p <parents_directory>  -i <input_tsv_file> -l <lca_file> -o <output_file> -s <summary_file> -g <genes_file>")
			sys.exit()
			...
			GENES_FILE = arg
	# end for
	return FS_ID, PARENTS_DIR, INPUT_TSV_FILE, LCA_FILE, OUTPUT_FILE, SUMMARY_FILE, GENES_FILE
# end parseArgv
...
def parseTsv(tsvFile, distPS, dictLCA, outputFile, printClusters, summaryFile, genesFile, printGenes, MAX_PS):
...
if __name__ == "__main__":
	...
	FS_ID, PARENTS_DIR, INPUT_TSV_FILE, LCA_FILE, OUTPUT_FILE, SUMMARY_FILE, GENES_FILE = parseArgv(sys.argv[1:])
	...
	if GENES_FILE != "":
		...
		printGenes = True
	parseTsv(tsvFile, dictPS, dictLCA, outputFile, printClusters, summaryFile, genesFile, printGenes, MAX_PS)
