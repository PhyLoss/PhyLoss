...
"""
The phylostratum_function_enrichment_analysis_v3.py module performs an enrichment analysis of functions gained and lost per
...
"""
# Usage: python3 phylostratum_function_enrichment_analysis_v3.py -i INPUT_FOLDER -s H -C -c 0.8 -p -o OUTPUT_FOLDER \
#																	-t PATH_TO_COG_FILE/COGs.txt -P PS_NAMES_FOLDER
...
INPUT_SPECIES_LONG = {"H": "Homo_sapiens_9606", "A" : "Arabidopsis_thaliana_3702", \
						"D" : "Drosophila_melanogaster_7227", "S" : "Saccharomyces_cerevisiae_559292"}
...
def parseArgv(argv):
	...
	options = "hi:s:GCc:po:t:P:" # options that require arguments should be followed by :
	long_options = ["help", "input_directory", "species", "GO_function", "COG_function", "c_value", "plot", \
										"output_directory", "path-categories-file", "pyhlostrata_directory"]
	try:
		opts, args = getopt.getopt(argv, options, long_options)
	except getopt.GetoptError:
		print("Error. Usage: phylostratum_function_enrichment_analysis_v3.py -i <input_directory> -s <species> -G[|-C]" + \
											" -c <c_value> [-p] -o <output_directory> -t <path-categories-file> -P <pyhlostrata_directory>")
		sys.exit(2)
	...
	OUTPUT_DIR = ""
	CATEGORY_INFO_PATH = ""
	PHYL_NAMES_FOLDER_PATH = ""
	
	for opt, arg in opts:
		print(opt + "\t" + arg)
		if opt in ("-h", "--help"):
			print("Usage: phylostratum_function_enrichment_analysis_v3.py -i <input_directory> -s <species> -G[|-C] -c <c_value> [-p]" + \
																		" -o <output_directory> -t <path-categories-file> -P <pyhlostrata_directory>")
			sys.exit()
		elif opt in ("-s", "--species"):
			if (arg in "ADHS"):		
				SPECIES = arg
			else:		
				print("Available species: A(rabidopsis thaliana), D(rosophila melanogaster), H(omo sapiens), S(accharomyces cereivisae)")
				sys.exit()
			...
			OUTPUT_DIR = arg
		elif opt in ("-t", "--path-categories-file"):
			CATEGORY_INFO_PATH = arg
		elif opt in ("-P", "--pyhlostrata_directory"):
			PHYL_NAMES_FOLDER_PATH = arg
	# end for
	
	return SPECIES, COG, C_VALUE, PLOT, INPUT_DIR, OUTPUT_DIR, CATEGORY_INFO_PATH, PHYL_NAMES_FOLDER_PATH 
# end parseArgv
...
def hypergeometric_over_under(quant, sample, hit, total):
	"""
	 ...
	 :return: over p-value,
	 """
	#over_rep = hypergeom.sf(quant - 1, total, hit, sample)# MDL: why -1??
	over_rep = hypergeom.sf(quant, total, hit, sample)# MDL: why -1??
...
def perform_phylostratum_hyper_test(total_count_path, sample_count_path, out_txt_path,
	...
	for cat in cat_2_plot_data:
		...
		plt.grid(axis='both', linestyle='-', linewidth=0.6) # MDL changed February, 2021
...
if __name__ == "__main__":
	...
	SPECIES, COG, C_VALUE, PLOT, INPUT_DIR, OUTPUT_DIR, CATEGORY_INFO_PATH, PHYL_NAMES_FOLDER_PATH = parseArgv(sys.argv[1:])
		...
		for g_l in gain_loss:
			...
			if mul == "voting":
				...
				if g_l:					
					gain = perform_phylostratum_hyper_test(total_count_path, sample_count_path, out_path_txt,
															out_path_plot, gain_loss=g_l, go_path=cat_path,
															phyl_name_path=path_phyl_2_name, voting=False, cog=COG)
				else:
					loss = perform_phylostratum_hyper_test(total_count_path, sample_count_path, out_path_txt,
													 out_path_plot, gain_loss=g_l, go_path=cat_path,
													 phyl_name_path=path_phyl_2_name, voting=False, cog=COG)
