...
"""
# Usage: nohup time python3 phylostratum_function_enrichment_analysis_v2.py -s H -C -c 0.8 -p > clu_funct_analysis_HS_2022_c_0_8_cog.out &
...
def parseArgv(argv):
    ...
    options = "hs:GCNc:p" # options that require arguments should be followed by :
    long_options = ["help", "species", "GO_function", "COG_function", "c_value", "plot"]
    try:
        opts, args = getopt.getopt(argv, options, long_options)
    except getopt.GetoptError:
        print("Error. Usage: phylostratum_function_enrichment_analysis_mdl_v2.py -s <species> -G[|-C] -c <c_value> [-p]")
        sys.exit(2)
    ...
    for opt, arg in opts:
        print(opt + "\t" + arg)
        if opt in ("-h", "--help"):
            print("Usage: phylostratum_function_enrichment_analysis_mdl_v2.py -s <species> -G[|-C] -c <c_value> [-p]")
            sys.exit()
    ...
    return SPECIES, COG, C_VALUE, PLOT
# end parseArgv
...
if __name__ == "__main__":
    ...
    SPECIES, COG, C_VALUE, PLOT = parseArgv(sys.argv[1:])
    STR_PARAMETER_C = "c_0_" + str(C_VALUE)[2].rstrip()

    CLUSTER_IN_PATH = "PATH_TO_THE_ANALYSIS_FOLDER/Clusters_2022/" + STR_PARAMETER_C + "/" \
                        + INPUT_SPECIES[SPECIES] + "_clusters.txt" # "/HomoSapiens_2020_clusters.txt"
    ...
    if COG:
        ...
        OUTPUT_FOLDER_PATH = "PATH_TO_THE_ANALYSIS_FOLDER/results_2022/phylostratum_hyper_go_v2/" + STR_PARAMETER_C + "/"
        category_type = " (GO)"
        filename_end = "_go"
