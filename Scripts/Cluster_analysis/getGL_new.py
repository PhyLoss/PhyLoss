...
"""
Usage: time python3 getGL_2.py -f 9606 -p YOUR_PATH/Data/Parents/ -i YOUR_PATH/Data/results_0_0/db_clu_all.tsv -l YOUR_PATH/Data/allLCA.txt \
									-o YOUR_PATH/Data/res_dbAll_H_sap_0_0.txt -s YOUR_PATH/Data/summary_GL_H_sap_0_0.txt
Note: -o option is optional; if only needed summary data, use only -s option!
"""
...
def parseArgv(argv):
	...
	try:
		...
		print("Error. Usage: getGL_new.py -f <focal_species_id> -p <parents_directory> -i <input_tsv_file> -l <lca_file> -o <output_file> -s <summary_file>")
		sys.exit(2)
	...
	for opt, arg in opts:
		print(opt + "\t" + arg)
		if opt in ("-h", "--help"):
			print("Usage: getGL_new.py -f <focal_species_id> -p <parents_directory>  -i <input_tsv_file> -l <lca_file> -o <output_file> -s <summary_file>")
			sys.exit()
...
def parseTsv(tsvFile, distPS, dictLCA, printClusters, outputFile, summaryFile, MAX_PS):
	...
	for line in tsvFile: # representativeID taxID
		...
		if reprId == geneId: # new cluster (the first memeber of the cluster is its representative)
			...
			if clusterId > 0: 
				...
				# print only those clusters that include the focal species and have at least 2 members or whose minPS != maxPS (GF was gained/lost along the way to the focal species)
...
if __name__ == "__main__":
	...
	parseTsv(tsvFile, dictPS, dictLCA, printClusters, outputFile, summaryFile, MAX_PS)
