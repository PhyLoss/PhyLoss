...
(1) database that contains proteins in fasta format (a fasta file for each species in the database)
...
(5) clusters.tar.gz
		- the clusters computed using MMseqs2 and the phylogeny data (data (1)-(4)) 		

(6) eggnog.tar.gz
	- the precomputed genes functional annotations (COG and GO annotations) for all genomes in the database:
		- extract to the subfolder "PATH_TO_ANALYSIS_FOLDER/Data/eggnogResults"

(7) go.obo, go-basic.obo (http://geneontology.org/docs/download-ontology/)
		- GO and GO-basic functions file

(8) COGs.txt (http://clovr.org/docs/clusters-of-orthologous-groups-cogs/)
...
(1) create the subolder "DB" in the analysis folder 
...
(5) creating list of parents for each focal species (in "Parents" subdirectory);
	first edit treeTaxId.awk, so to incorporate nodes.txt in the code (see the source code of "treeTaxId.awk")
	>> bash computeParents.sh nodes.txt Parents > computeParents.log
	>> cat Parents/*parents.txt > allParents.txt 
	
	NOTE: 
		computeParents.sh calls treeTaxId.awk
	Output:
		- the list of files in the subfolder "Parents" - a file for each taxID, e.g. "Parents/9606parents.txt" and "allParents.txt"
...
(6) computing LCA (Last Common Ancestor) matrix for all n taxID (n x n); taxon's names are truncated to 10 char-s:
	>> awk -f getAllLCA.awk allParents.txt > allLCA.txt

(7) for a focal species: 
		compute phylostrata for each species (taxID) in the database
	
	>> python3 getAllPS.py -f 9606 -i allParents.txt -p ./Parents -o taxId_ps_Hsap.txt

(8) For each c value (0.0 to 0.8):
	...
	Example for c = 0.8:
		- the output folder (here "./results_0_8/") should have a subfolder for a focal species (e.g. "Hsap")
		>> python3 numClusters.py -i names.txt -j . -t ./results_0_8/db_clu_all.tsv -o ./results_0_8/
		
	Output: 
		an output file for each focal species< e. g. for S. cerevisiae:
		-------------	
			(1) Cluster representative: xxxxxxxxxxxxxxxx      Number of members in the cluster: 2
					Cluster members:
					(1) xxxxxxxxxxxxxxxx
					Dipodomys_ordii 9
					(2) yyyyyyyyyyyyyyyy
		...
		-------------
			...
			where
				the protein xxxxxxxxxxxxxxxx belongs to Mus_musculus (taxID = 10090), 
					which is in the phylostratum 9 from the perspective of the focal species S. cerevisiae
	
(9) form a matrix of LCA, with the number of taxa at the start of the file, e.g. if the database contains n = 100 taxa:
	>> echo "100" >> allLCA_n_100.txt
	>> cat allLCA.txt >> allLCA_n_100.txt

(10) For each focal species:
	...
	Note: 
		- a sge script uses an accompanying awk scipt (getGL.awk)
		- in each script replace "PATH_TO_ANALYSIS_FOLDER" with the path to the analysis folder
	>> bash numCluster_HS.sge & 

	Output files (for each focal species and each c-value): gene cluster gain/loss, 
		and an auxiliary results file containing phylostratum for each gene in each cluster
...
(1) to compute the clusters using parseClusters_for_FA_NEW.py (output folder: Clusters_2022)
	...
	Output:
		...
		- the first row contains a geneID (a cluster representative)
